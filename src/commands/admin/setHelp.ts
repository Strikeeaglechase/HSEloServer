...
class SetHelp extends SlashCommand {
	name = "sethelp";
	description = "Sets help text for a category";
	...
	async run({ interaction, framework, app }: SlashCommandEvent<Application>, @SArg({ autocomplete: true }) category: string) {
		...
		collector.on("collect", async (msg) => {
			...
			if (text.toLowerCase() === "delete") {
				const result = await app.serverHelps.collection.deleteOne({ id: category });
				...
				await app.serverHelps.collection.updateOne(
					{ id: category },
				...
				const successEmbed = new EmbedBuilder()
					.setTitle("Success")
					.setDescription(`Updated **${category}** help with:\n\n${text}`)
					.setColor(0x00ff00);
