...
interface Kill {
  killer: { ownerId: string; type: string; weapon: Weapon };
  victim: { ownerId: string; type: string };
  time: number;
  weapon: Weapon;
  // Add any other properties you use from kills/deaths
}
...
const MAX_SESSION_MS = 1000 * 60 * 60 * 1.1; // 1.1 hours
function getKillsPerHour(user: User, kills: Kill[], season: { startTime: number; endTime: number }): string {
  if (!user.sessions || user.sessions.length === 0) return "0.00";
  const seasonSessions = user.sessions.filter(
    (session: UserSession) =>
      session.startTime &&
      session.endTime &&
      session.startTime >= season.startTime &&
      session.startTime <= season.endTime &&
      session.endTime > session.startTime &&
      session.endTime - session.startTime <= MAX_SESSION_MS
  );
  const sessionsToUse = seasonSessions.length > 0 ? seasonSessions : user.sessions.filter(
    (session: UserSession) =>
      session.startTime &&
      session.endTime &&
      session.endTime > session.startTime &&
...
function getFirstOnline(user: User): string {
  const sessionTimes =
    user.sessions?.map((s) => s.startTime).filter((t) => Number.isFinite(t) && t > 0) || [];
  const loginTimes =
    user.loginTimes?.filter((t) => Number.isFinite(t) && t > 0) || [];
  const allTimes = [...sessionTimes, ...loginTimes];
  if (allTimes.length === 0) return "Never";
  return new Date(Math.min(...allTimes)).toLocaleDateString();
}

function getLastOnline(user: User): string {
  if (!user.sessions || user.sessions.length === 0) return "Never";
...
function getTotalOnlineHours(user: User): string {
  if (!user.sessions || user.sessions.length === 0) return "0.00";
  const totalOnlineMs = user.sessions.reduce((acc, session) => {
    if (
      session.startTime &&
      session.endTime &&
      session.endTime > session.startTime
  ...
  return (totalOnlineMs / 1000 / 60 / 60).toFixed(2);
}

function getAvgSessionLength(user: User, season: { startTime: number; endTime: number }): string {
  ...
  if (sessionsToUse.length === 0) return "Avg. Session Length: N/A";
...
function getAircraftStats(
  ...
  kdr: string | number;
  killsPerHour: string;
  weaponKills: { weapon: string; count: number }[];
}[] {
  const aircraftList = [
  ...
  const aircraftStatsMap: Record<string, { kills: Kill[]; deaths: Kill[] }> = {};
    ...
    const acSeasonSessions = user.sessions
      ? user.sessions.filter(
          (session: UserSession) =>
            ...
            session.startTime >= targetSeason.startTime &&
            session.startTime <= targetSeason.endTime &&
            session.endTime > session.startTime &&
            session.endTime - session.startTime <= MAX_SESSION_MS
        )
      : [];
...
class Stats extends SlashCommand {
  ...
  async run(
    ...
    let kills: Kill[] = await app.kills.collection
      .find({ "killer.ownerId": user.id, season: targetSeason.id })
      .toArray();
    let deaths: Kill[] = await app.kills.collection
    ...
    // Aircraft stats
    const aircraftStats = getAircraftStats(user, kills, deaths, targetSeason);

    // Weapon stats
    const weaponKillsStr = getWeaponStats(kills);
    const weaponDeathsStr = getWeaponStats(deaths);
    ...
    embed.addFields([
      {
        ...
        value: [
          ...
          `Avg: ${Math.floor(
            user.eloHistory.reduce((sum, h) => sum + h.elo, 0) /
              user.eloHistory.length
          )}`,
        ].join("\n"),
        inline: true,
      },
      ...
      },
      {
        ...
        value: [
          ...
          `Last Online: ${getLastOnline(user)}`,
          `Online Time: ${getTotalOnlineHours(user)} hours`,
        ].join("\n"),
        inline: true,
      },
      {
        name: "Aircraft Kills",
        value: aircraftStats
          .map((ac) => `${ac.label}: ${ac.weaponKills.reduce((sum, w) => sum + w.count, 0)}`)
          .join("\n") || "<No Data>",
        inline: true,
      },
      ...
      {
        name: "Kills against",
        value: Object.entries(killsPerVictim)
      ...
      {
        name: "Deaths against",
        value: Object.entries(deathsPerKiller)
          .map(([id, count]) => `${id}: ${count}`)
          .join("\n"),
        inline: true,
      },
      {
        ...
        value: [
          ...
          getAvgSessionLength(user, targetSeason.id),
        ].join("\n"),
        inline: true,
      },
      // Per-aircraft stats
    ...
    if (achievementsEnabled && (targetSeason.active || endOfSeasonStats)) {
      ...
      const dbAchievements = await Promise.all(
        achievements.map((ach) => {
          ...
          return targetSeason.endStats.achievementHistory.find(
            (a) => a.id == ach.id
          );
        })
      );
      ...
      const optBold = (
        entry: { txt: string; bold: boolean },
        pad: number
      ) => {
        if (!entry.bold) return entry.txt.padEnd(pad);
        return `[2;37m[1;37m${entry.txt.padEnd(
          pad
        )}[0m[2;37m[0m`;
      };
      const col0Pad =
        Math.max(...table.map((e) => e[0].txt.length)) + 3;
      table.forEach((row) => {
